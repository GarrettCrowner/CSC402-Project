...
def handle_guided_flow(message: str, history: list) -> Optional[str]:
    ...
    # Include the current message so chip answers are recognized immediately
    ...
    if intent == "health_insurance":
        ...
        return None  # All info collected — fall through to ask_model

    if intent == "retirement":

        has_selection = any(option in full_text for option in [
            "permanent full-time", "temporary", "faculty", "750"
        ])
...
def clear_conversation():
    global GLOBAL_HISTORY, FLOW_STATE
